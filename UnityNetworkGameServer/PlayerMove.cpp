...
void PlayerMove::ChangeState(State state)
{
	...
	case PlayerMove::State::Die:
	{
		...
		RPC("DieInformation", gameObject->owner, lastHitPlayerID, myKillCount, killersKillCount, battleFieldManager->playerRespawnDelay, MinNetTime::curTime());// 죽고죽인 횟수를 보여주면 경쟁하는 재미가 생기지 않지만 넣음
...
void PlayerMove::Update()
{
	...
	{
		if (MinNetTime::curTime() - lastHit > hitResetTime)
		{
			HitInformationReset();
		}
	}
...
bool PlayerMove::IsCanRespawn(clock_t playerRespawnDelay)
{
	if (state == State::Die)
	{
		if (MinNetTime::curTime() - dieTime > playerRespawnDelay)
			return true;
	}
	return false;
}
